  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch("http://localhost:8080/api/templates", {
        ...
        // On envoie la complexit\u00e9 par d\u00e9faut (1) m\u00eame si c'est disable
        body: JSON.stringify({ name, columns: columns.map(c => ({name: c, type:"TEXT"})), complexity }), 
      });
                 ...
                 <div className={styles.cartridgeInfo}>
                     <div>
                         ...
                         <div className={styles.cMeta}>
                            ...
                            <Layers size={12} /> {t.fields?.length || 0} MODULES
                         </div>
                     </div>
                 </div>
