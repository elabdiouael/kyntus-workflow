...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      // Simulation délai biométrique (Juste pour le feeling)
      ...
      if (res.ok) {
        ...
        // 1. Sauvegarde Session (Crucial)
        localStorage.setItem("kyntus_user", JSON.stringify(user));
        
        // 2. ROUTING INTELLIGENT (SÉPARATION DES MONDES)
        setTimeout(() => {
            if (user.role === "ADMIN") {
                // Admin -> Ymchi ychouf l'3jb (Portal 3D) 3ad ydkhol
                router.push("/portal"); 
            } else if (user.role === "PILOT") {
                // Pilote -> Nishan l'khedma (Tactical Board). Pas de temps à perdre.
  ...
  return (
    <div className={styles.container}>
      ...
      {/* Background léger pour pas bouffer le GPU au login */}
